...
## Input: MPS file
## Output: a list with elements c A dir b max int bounds 
...
Rglpk_read_MPS <- function(file, type = "Standard", verbose=FALSE) {
  ...
  obj <- Rglpk_call_MPS_reader(obj, verbose)
  out <- Rglpk_retrieve_MILP_from_MPS(obj, verbose)
  class(out) <- "MILP"
  out
}
...
Rglpk_call_MPS_reader <- function(x, verbose = FALSE){
...
Rglpk_retrieve_MILP_from_MPS <- function(x, verbose = FALSE){
